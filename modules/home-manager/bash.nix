{ lib, ... }: {
    programs.bash = {
        ...
        shellAliases = {
            ...
            "gamingcont" = "sudo systemd-nspawn -q -D ~/Files/Containers/gaming -E DISPLAY=$DISPLAY -E XDG_RUNTIME_DIR=$XDG_RUNTIME_DIR --bind=/dev/input --bind=/dev/dri --bind=/dev/shm --bind=/etc/machine-id --bind=/run/dbus/system_bus_socket --bind=/tmp/.X11-unix --bind=/dev/snd -E PULSE_SERVER=unix:$XDG_RUNTIME_DIR/pulse/native --bind=$XDG_RUNTIME_DIR --bind=$HOME/.config/pulse/cookie:/home/gamer/.config/pulse/cookie"; # -u gamer
        };
        initExtra = ''
